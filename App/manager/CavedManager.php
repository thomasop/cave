...
class CavedManager extends Manager
{

    public function all()
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare('SELECT id, nom, quantite, annee FROM cave_d');
        $bdGetAll->execute();
    ...
    public function allPosition()
    {
        ...
        $bdGetAll = $bd->prepare(
            ...
            FROM cave_d c
            JOIN position_cave_d a ON c.id = a.cave_d_id
            JOIN position_d p ON a.position_id = p.id'
        );
        $bdGetAll->execute();
        $result = $bdGetAll->fetchAll(PDO::FETCH_ASSOC);
        return $result;  
    ...
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare('SELECT * FROM cave_d WHERE slug = ? AND annee = ?');
    ...
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare('SELECT id FROM position_d WHERE colonne = ? AND ligne = ?');
    ...
    public function deletePosition($colonne, $ligne)
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare(
            'DELETE FROM position_d
            WHERE position_d.colonne = ?
            AND position_d.ligne = ?'
        );
        $bdGetAll->execute(array($colonne, $ligne));
        return $bdGetAll;  
    }
    ...
    public function addCave($nom, $appellation, $annee, $type, $region, $contenance, $slug, $pays)
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare(
            'INSERT INTO cave_d(nom, appellation, annee, typev, region, contenance, slug, pays) VALUES(?, ?, ?, ?, ?, ?, ?, ?)'
        );
    ...
    public function addPosition($colonne, $ligne)
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare(
            'INSERT INTO position_d(colonne, ligne) VALUES(?, ?)'
        );
        $bdGetAll->execute(array($colonne, $ligne));
        return $bdGetAll;  
    }
    ...
    public function add($nom, $annee)
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare(
            'INSERT INTO position_cave_d(position_id, cave_d_id) VALUES(?, ?)'
        );
    ...
    public function updateQtn($id, $qtn)
    {
        $bd = $this->connection();
        $bdGet = $bd->prepare(
            'UPDATE cave_d SET quantite = :qtn WHERE id = :id'
        );
    ...
    public function update($id, $nom, $appellation, $annee, $typev, $region, $contenance, $slug, $pays)
    {
        $bd = $this->connection();
        $bdGet = $bd->prepare(
            'UPDATE cave_d SET nom = :nom, appellation = :appellation, annee = :annee, typev = :typev, region = :region, contenance = :contenance, slug = :slug, pays = :pays WHERE id = :id'
        );
        $bdGet->bindValue(':id', (int)$id["id"], PDO::PARAM_INT);
