...
class CavebManager extends Manager
{
    ...
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare('SELECT id, nom, quantite, annee FROM cave_b');
        $bdGetAll->execute();
    ...
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare('SELECT * FROM cave_b WHERE slug = ? AND annee = ?');
    ...
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare('SELECT id FROM position_b WHERE colonne = ? AND ligne = ?');
    ...
    public function deletePosition($colonne, $ligne)
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare(
            'DELETE FROM position_b
            WHERE position_b.colonne = ?
            AND position_b.ligne = ?'
        );
        $bdGetAll->execute(array($colonne, $ligne));
        return $bdGetAll;  
    }
    ...
    public function addCave($nom, $appellation, $annee, $type, $region, $contenance, $slug, $pays)
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare(
            'INSERT INTO cave_b(nom, appellation, annee, typev, region, contenance, slug, pays) VALUES(?, ?, ?, ?, ?, ?, ?, ?)'
        );
    ...
    public function addPosition($colonne, $ligne)
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare(
            'INSERT INTO position_b(colonne, ligne) VALUES(?, ?)'
        );
        $bdGetAll->execute(array($colonne, $ligne));
        return $bdGetAll;  
    }
    ...
    public function add($nom, $annee)
    {
        $bd = $this->connection();
        $bdGetAll = $bd->prepare(
            'INSERT INTO position_cave_b(position_id, cave_b_id) VALUES(?, ?)'
        );
    ...
    public function updateQtn($id, $qtn)
    {
        $bd = $this->connection();
        $bdGet = $bd->prepare(
            'UPDATE cave_b SET quantite = :qtn WHERE id = :id'
        );
    ...
    public function update($id, $nom, $appellation, $annee, $typev, $region, $contenance, $slug, $pays)
    {
        $bd = $this->connection();
        $bdGet = $bd->prepare(
            'UPDATE cave_b SET nom = :nom, appellation = :appellation, annee = :annee, typev = :typev, region = :region, contenance = :contenance, slug = :slug, pays = :pays WHERE id = :id'
        );
        $bdGet->bindValue(':id', (int)$id["id"], PDO::PARAM_INT);
